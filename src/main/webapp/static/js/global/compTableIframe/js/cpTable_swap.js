...
CpTable.prototype = {
	...
	refreshTable_swap : function(data){
		for(var i=0;i<this.curPairsName.length;i++){
			var curPairName = this.curPairsName[i];
			var code = data[0].code;
			if(code=='abs'){
				if(curPairName=='gold'){
					curPairName='xauusd';
				}
				if(curPairName=='silver'){
					curPairName='xagusd';
				}
				curPairName+='abs';
			}
			...
			if(i==0){
				for(var j =0;j<data.length;j++){
					...
					if(data[j].currencyPair==curPairName){
						var swapSymbol = data[j].swapSymbol;
						var longSwap = data[j].longswap;
						var shortSwap = data[j].shortswap;
						if(swapSymbol=='pips' && code!='fibo'){
							longSwap/=10;
							shortSwap/=10;
						}
						if(data[j].longSwapIntact.substr(0,1)!="-"){
							d[1].lastChild.innerHTML="&thinsp;&thinsp;"+longSwap.toFixed(3)+swapSymbol;							
						}else{
							d[1].lastChild.innerHTML=longSwap.toFixed(3)+swapSymbol;
						}
						if(data[j].shortSwapIntact.substr(0,1)!="-"){
							d[2].lastChild.innerHTML="&thinsp;&thinsp;"+shortSwap.toFixed(3)+swapSymbol;
						}else{
							d[2].lastChild.innerHTML=shortSwap.toFixed(3)+swapSymbol;							
						}
					}
				}
			}else{
				for(var j =0;j<data.length;j++){
					...
					if(data[j].currencyPair==curPairName){
						...
						if(data[j].longSwapIntact.substr(0,1)!="-"){
							d[0].lastChild.lastChild.innerHTML="&thinsp;&thinsp;"+longSwap.toFixed(3)+swapSymbol;					
						}else{
							d[0].lastChild.lastChild.innerHTML=longSwap.toFixed(3)+swapSymbol;
						}
						if(data[j].shortSwapIntact.substr(0,1)!="-"){
							d[1].lastChild.lastChild.innerHTML="&thinsp;&thinsp;"+shortSwap.toFixed(3)+swapSymbol;
						}else{
							d[1].lastChild.lastChild.innerHTML=shortSwap.toFixed(3)+swapSymbol;						
						}
					}
				}
			}
    ...
    initTable : function(self){
            ...
            //头部第二列货币点差
                ...
                if(this.headHasHref == "yes"){
                    if(this.titleType == "blue"){
                        thHtml = '<a href="javascript:void(0);" target="_blank"style="text-decoration:none;color:#367cd3;font-weight:normal;">';
                    }else if(this.titleType == "yellow"){
                        th.style.color = "#3897e6";
                        thHtml = '<a href="javascript:void(0);" target="_blank"style="text-decoration:none;color:#3897e6;">';
                    }
                }else{
                    if(this.titleType == "blue"){
                ...
                if(this.titleDCText == "yes"){
                    thHtml += '<p style="font-weight:normal;margin:0;padding:0;font-size:12px:color:#3897e6;">(点差)</p>';
                }
                thHtml+="</a>";
                th.colSpan = "2";
                th.innerHTML = thHtml;
        ...
        for(var i = 0;i < this.mData.length;i++){
            ...
            for(var j=0;j<this.curPairsName.length+6;j++){
                ...
                //table内容行第二列，买入，卖出列
                    ...
                    divLeft.innerHTML = "<a href='../../broker/dianChaCompare' target='_blank' style='border-bottom:1px solid #3897e6;text-decoration: none;font-size:15px;color:#3897e6'>点差</a>";
		...
		if(this.mData.length==1){
			...
			td1.colSpan="2";               
					var divLeft = document.createElement('div');
                    divLeft.style.textAlign = "center";
                    divLeft.innerHTML = "<a href='../../broker/liXiCompare' target='_blank' style='border-bottom:1px solid #3897e6;text-decoration: none;font-size:15px;color:#3897e6'>利息</a>";
                    divLeft.style.height = "57px";
                    divLeft.style.lineHeight = "55px";
                    divLeft.style.color = "#666";
                    divLeft.style.width = "50px";
