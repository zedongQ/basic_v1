...
public class MailUtil {
	...
	private static boolean publicsendEmail(String userName,String password,String targetAddress,
		...
		try {
			publicsendEmailEx(userName, password, targetAddress, mimeDTO, isGroup, filepath);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public static void publicsendEmailEx(String userName,String password,String targetAddress,
			MimeMessageDTO mimeDTO,boolean isGroup,List<String> filepath) throws Exception{
	...
	private static Properties makeMailProperties(String userName){
		...
		String hostname=SMTPUtil.getSMTPAddress(userName);
		props.put("mail.smtp.host", "ssl://"+hostname);
	...
	private static Message createEmailByGroupAndFile(Session session,String userName,
								String regMail,MimeMessageDTO mimeDTO,List<String> filepath) throws UnsupportedEncodingException{
		MimeMessage message = new MimeMessage(session);
		try { 
			String nick = "";    
	        try {    
	            nick = javax.mail.internet.MimeUtility.encodeText("荔枝返现网");    
